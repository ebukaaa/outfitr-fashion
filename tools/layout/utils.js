...
export function useStore({ contentStyle, containerStyle }) {
  ...
  return {
    ...
    }, [height, containerStyle, contentStyle]),
    Animated,
    View,
    TouchableWithoutFeedback,
    AnimatedKeyboardAvoidingView: useMemo(
      () => memo(Animated.createAnimatedComponent(KeyboardAvoidingView)),
      []
    ),
    Cover: useMemo(() => {
      function useCover({ style: coverStyles }) {
        return (
          <Svg
            height={0.28 * height}
            viewBox="1 1 375 191"
      ...
      return memo(useCover);
    }, [height, width]),
    Footer: useFooter,
    onDismissKeyboard: useCallback(() => Keyboard.dismiss(), []),
  };
}
